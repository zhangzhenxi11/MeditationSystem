...
void Myfilter::add_index_range(vectori &indices, int beg, int end, int inc)
{
    for (int i = beg; i <= end; i += inc)
        indices.push_back(i);
}

void Myfilter::add_index_const(vectori &indices, int value, size_t numel)
{
    while (numel--)
        indices.push_back(value);
}

void Myfilter::append_vector(vectord &vec, const vectord &tail)
{
    vec.insert(vec.end(), tail.begin(), tail.end());
}

vectord Myfilter::subvector_reverse(const vectord &vec, int idx_end, int idx_start)
{
...
int Myfilter::max_val(const vectori &vec)
{
