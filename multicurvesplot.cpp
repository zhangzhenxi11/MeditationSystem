...
//    this->yAxis2->rescale(true);//y轴值自动变化
...
//    connect(xAxis,SIGNAL(rangeChanged(QCPRange)),xAxis2,SLOT(setRange(QCPRange)));//上下x轴同步放缩
...
//    qDebug() << "MultiCurvesPlot::showCurves:" << idxList;
    ...
    {   //*it为每一个要显示的曲线编号
        ...
        //自动缩放坐标轴 y轴
//        this->graph(graphIdx)->rescaleValueAxis();
//        this->yAxis->rescale(true);//y轴值自动变化
        graphIdx++;
...
//    if(y > 1e10 || y < -1e10)//接收到的异常值直接不绘制  1乘以10的10次方
//        return;
